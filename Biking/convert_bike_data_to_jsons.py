...
def main():
    ...
    input_file = open("bike_data.csv", "r")
    for line in input_file:
        ...
        # Speed data is just put into two lists: one for the dates (will be used
        # as labels) and one for the average speed per tour:
        speed_data_dates.append(columns[0] + "-" + columns[1] + "-" + columns[2])  # e.g. "2020-12-24"
        speed_data_speeds.append(columns[4].replace(",", "."))        
    ...
    output_file = open("bike_data_distance.json", "w")
    output_file.write(json_data)
    output_file.close()

    # The speed data is much simpler: Basically, the lists from
    # above are just combined into one dataset and the corresponding
    # data object:
    dataset = {}
    dataset["label"] = "Average Speed"
    dataset["data"] = speed_data_speeds
    dataset["fill"] = False
    dataset["lineTension"] = 0
    dataset["borderColor"] = "rgba(255, 0, 0, 0.6)"

    datasets = []
    datasets.append(dataset)
    
    bar_chart_speed_data = {}
    bar_chart_speed_data["datasets"] = datasets
    bar_chart_speed_data["labels"] = speed_data_dates

    json_data = json.dumps(bar_chart_speed_data)

    output_file = open("bike_data_speed.json", "w")
    output_file.write(json_data)
    output_file.close()
